...
COPY . .
ARG CARGO_BUILD_TARGET=must-set-CARGO_BUILD_TARGET
RUN \  
...
RUN if echo $CARGO_BUILD_TARGET | grep -q linux; then file target/cross/release/pup | tee /dev/stderr | grep -qE 'static(ally|-pie) linked'; fi

FROM scratch as final
ARG EXE_SUFFIX=
COPY --from=builder /usr/src/pup/target/cross/release/pup$EXE_SUFFIX /pup
ENTRYPOINT ["/pup"]
